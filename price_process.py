...
def plot_bank(e_bar, alpha, S, theta, eta, r):
	...
	T_max = int(total_p_increase*(1-eta)/r)
	t = np.arange(0, T_max+PRESTART+10)-PRESTART

	p = price_function(pT, P0, R, eta, t, T_max)
	e = emission_function(P0, p, e_bar, theta)
	b = banking(t, B0, PHI, ALPHA, S, e_bar, theta, eta, R, 0)

	banking_end = solve_b_zero_times(PHI, B0, theta, R, eta, 0, e_bar, ALPHA, S)
	print(banking_end)
	#plot(t, p, b, e, alpha*S, pT)

	T_max_float = total_p_increase*(1-eta)/r
	DeltaT = solve_DeltaT_match_roof(T_max_float, PHI, B0, theta, R, eta, e_bar, ALPHA, S)
	if DeltaT is None:
		DeltaT = 0
	T_max = T_max - int(DeltaT)
...
def plot(t, p, b, e, alpha_S, pT,root=None):
	...
	fig.legend(["Price","$p_T$", "Banking", "Emissions", "Issuance"],  frameon=False, 
			loc="upper right", bbox_to_anchor=(0.9, 0.5))
	#plt.ylim(-0.5, 1.1)
